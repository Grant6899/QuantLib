...
// ++++++++++++++++++ Description of the discrete dividends
	...
	// Last know dividend:
	Real				dividend			= 0.0;//0.75;		// INPUT
	Date				nextDividendDate	(10,Feb,2012);		// INPUT
	// HERE we have make the assumption that the dividend will grow with the quarterly croissance:
	Real				dividendCroissance	= 1.03;				// INPUT
	Period				dividendfrequence	= 3*Months;			// INPUT
	std::vector<Date> dividendDates;
	std::vector<Real> dividends;
	for (Date d=nextDividendDate; d<maturity; d += dividendfrequence ){
		dividendDates.push_back(d);
		dividends.push_back(dividend);
		dividend*=dividendCroissance;
	}
	std::cout << "Discrete dividends				" << std::endl;
	std::cout << "Dates				Dividends		" << std::endl;
	for (Size i=0; i<dividends.size(); i++)
		std::cout << dividendDates[i] << "		" << dividends[i] << std::endl;

// ++++++++++++++++++ Description of the final payoff 
	boost::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(type, strike));

// ++++++++++++++++++ The OPTIONS : (American and European) with their dividends description:
	DividendVanillaOption dividendEuropeanOption (payoff, europeanExercise, dividendDates, dividends);
	DividendVanillaOption dividendAmericanOption (payoff, americanExercise, dividendDates, dividends);
...
// just too test
	...
	boost::shared_ptr<PricingEngine> dividendEuropeanEngine(
...
// just to test
	...
	boost::shared_ptr<PricingEngine> americanEngine(
		new FDAmericanEngine<CrankNicolson>(bsProcess,americanTimeSteps, americanGirdPoints));
...
// just	to test
	...
	// ... and final results
	std::cout << "NPV of the European Option with discrete dividends=0:	"	<< dividendEuropeanOption.NPV() <<  std::endl;
	std::cout << "NPV of the European Option without dividend:		"		<< europeanOption.NPV() <<  std::endl;
	std::cout << "NPV of the American Option with discrete dividends=0:	"	<< dividendAmericanOption.NPV() <<  std::endl;	
	std::cout << "NPV of the American Option without dividend:		"		<< americanOption.NPV() <<  std::endl;
	//just a single test
