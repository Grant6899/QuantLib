...
// ++++++++++++++++++  Now the creation of the yield curve
	RelinkableHandle<YieldTermStructure> riskFreeTS ( boost::shared_ptr<YieldTermStructure> (
...
// just too test
    ...
    // timedependent = false
    boost::shared_ptr<PricingEngine> fdEuropeanEngine_timefalse(
    	new FDEuropeanEngine<CrankNicolson>(bsProcess, TimeSteps, GridPoints, false));
    ...
    // using FD method on the same option with timedependent = false
    europeanOption.setPricingEngine(fdEuropeanEngine_timefalse); 
    std::cout << "NPV of the European Option without dividend(FD Method time flase): " << europeanOption.NPV() << std::endl; 

    // get the zero rate at maturity
    InterestRate rT = riskFreeTS->zeroRate(maturity, riskFreeTS->dayCounter(), Continuous);
	std::cout << "ZeroRate at "<< maturity << ": " << rT << std::endl;

    // replace the term structure

    riskFreeTS.linkTo(boost::shared_ptr<YieldTermStructure>(
        new FlatForward(settlementDate, rT.rate(), riskFreeTS->dayCounter())));
    ...
    // get the zero rate at maturity
    rT = riskFreeTS->zeroRate(maturity, riskFreeTS->dayCounter(), Continuous);
	std::cout << "ZeroRate at "<< maturity << ": " << rT << std::endl;
